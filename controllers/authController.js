...
const createSendToken = (user, statusCode, res) => {
  ...
  const cookieOptions = {
    ...
    secure: true, // cookie will be sent only with https
    httpOnly: true, // cookie can not be modified by the browser
  };
  ...
  res
    .status(statusCode)
    .json({ status: 'success', token, body: { user: user } });
};
...
exports.login = catchAsync(async (req, res, next) => {
  ...
  // 3) If everything is OK, send back JWT to client
  createSendToken(user, 200, res);
});

exports.protect = catchAsync(async (req, res, next) => {
  // 1) Getting token and checking if it is present
  let token;
  if (
